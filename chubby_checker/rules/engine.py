...
class DiscrepancyEngine:
    ...
    def _check_thermal_blocks_verified(self):
        ...
        # Assume insulation present if thermal blocks or clips exist; can be refined
    ...
    # ------------------------------------------------------------------
    # Closures, Trim, Gutter, Downspout + geometry formulas
    # ------------------------------------------------------------------
    def _check_closures_trim_gutter(self):
        ...
        panel_count = sum(
            getattr(p, "quantity", 0) for c in panel_cats for p in categories.get(c, [])
        )
        ...
        # Geometry-based trim length formulas
        for f in check_trim_lengths_against_geometry(trim_info, self.geometry):
            self.discrepancies.append(Discrepancy(**f))

    # ------------------------------------------------------------------
    # Mark-by-mark / length / system flags (core)
    # ------------------------------------------------------------------
    ...
    def _check_panel_and_accessories(self):
        ...
        if dominant == "24" and plates_18 > plates_24 * 0.3 and plates_24 > 0:
            self.discrepancies.append(Discrepancy(
                severity="WARNING", category="Accessories",
                message=f"Unexpected 18\" backup plates ({plates_18}) on predominantly 24\" roof.",
